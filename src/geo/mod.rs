...
pub fn run_city_selection() -> anyhow::Result<(f64, f64)> {
    ...
    match calculate_civil_twilight_times_for_display(latitude, longitude, today) {
        Ok((sunset_time, sunset_start, sunset_end, sunrise_time, sunrise_start, sunrise_end, sunset_duration, sunrise_duration)) => {
            Log::log_block_start(&format!("Sun times for {} ({:.4}°, {:.4}°)", 
                city_name, latitude, longitude));
            
            // Display sunset info (happening today)
            Log::log_indented(&format!("Today's sunset: {} (transition from {} to {})",
                sunset_time.format("%H:%M"),
                sunset_start.format("%H:%M"),
                sunset_end.format("%H:%M")));
            
            // Display sunrise info (happening tomorrow)  
            Log::log_indented(&format!("Tomorrow's sunrise: {} (transition from {} to {})",
...
fn calculate_civil_twilight_times_for_display(
...
) -> Result<(chrono::NaiveTime, chrono::NaiveTime, chrono::NaiveTime, chrono::NaiveTime, chrono::NaiveTime, chrono::NaiveTime, std::time::Duration, std::time::Duration), anyhow::Error> {
    ...
    use crate::logger::Log;
    Log::log_indented(&format!("DEBUG - Sunrise UTC: {}, Local: {}, TZ: {}", 
        sunrise_utc.format("%H:%M"), sunrise_time.format("%H:%M"), timezone));
    Log::log_indented(&format!("DEBUG - Sunset UTC: {}, Local: {}, TZ: {}", 
        sunset_utc.format("%H:%M"), sunset_time.format("%H:%M"), timezone));
    ...
    let golden_hour_end = {
        let golden_hour_end_utc = tomorrow_solar_day
            .event_time(SolarEvent::Elevation {
                elevation: f64::to_radians(6.0),
                morning: true,
            });
    ...
    let use_fallback = is_polar_region && (civil_twilight_duration > 180 || civil_twilight_duration <= 0);
    ...
    let (sunset_start, sunset_end, sunrise_start, sunrise_end) = if use_fallback {
        ...
        let reasonable_golden_hour = golden_hour_start != sunset_time && 
                                   ...
                                   sunset_time.signed_duration_since(golden_hour_start).num_minutes() < 120;
        
        let reasonable_golden_hour_end = golden_hour_end != tomorrow_civil_dawn &&
                                       golden_hour_end > tomorrow_civil_dawn &&
                                       golden_hour_end.signed_duration_since(tomorrow_civil_dawn).num_minutes() < 120;
    ...
    Ok((
        ...
        sunset_duration.to_std().unwrap_or(std::time::Duration::from_secs(3600)),
        sunrise_duration.to_std().unwrap_or(std::time::Duration::from_secs(3600)),
    ))
}

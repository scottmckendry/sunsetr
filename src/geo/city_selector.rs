...
use crossterm::{
    cursor::{Hide, MoveTo, MoveUp, MoveDown, Show},
...
pub struct CityInfo {
    pub name: String,
    pub country: String,
    pub region: Option<String>, // State/province/region for disambiguation
    pub latitude: f64,
    pub longitude: f64,
...
pub fn select_city_interactive() -> Result<(f64, f64, String)> {
    ...
    Log::log_indented("Search for your city for accurate sunrise/sunset times");
...
fn get_all_cities() -> Vec<CityInfo> {
    let mut all_cities: Vec<CityInfo> = cities::all()
        .into_iter()
        .filter_map(|city| {
            ...
            // Try to get state/region info if available
            let region = None; // For now, we'll add this later when we figure out the crate structure
    ...
    let mut cities_with_distance: Vec<(CityInfo, f64)> = cities::all()
        ...
        .map(|city| {
            let city_info = CityInfo {
                name: city.city.to_string(),
                country: city.country.to_string(),
                region: None, // For now, we'll add this later
                latitude: city.latitude,
                longitude: city.longitude,
                population: None, // cities crate doesn't provide population data
            };
...
fn fuzzy_search_city(cities: &[CityInfo]) -> Result<&CityInfo> {
    ...
    let result = loop {
        ...
        if filtered_cities.is_empty() {
            ...
            stdout.execute(Print(format!("{} of {} cities", filtered_cities.len(), cities.len())))?;
        }
        stdout.execute(Print("\r\n"))?;
